...
import (
	"github.com/lancelote/writing-a-compiler-in-go/lexer"
	"github.com/lancelote/writing-a-compiler-in-go/object"
	"github.com/lancelote/writing-a-compiler-in-go/parser"
	"testing"
)
...
`,
	...
	for _, tt := range tests {
		...
		if !ok {
			t.Errorf("want error, got %T", evaluated)
			continue
		}

		if errObj.Message != tt.expectedMessage {
			t.Errorf("want error message: %s, got %s", tt.expectedMessage, errObj.Message)
		}
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("want function, got %T", evaluated)
	}

	if len(fn.Parameters) != 1 {
		t.Fatalf("want 1 parameter, got %d", len(fn.Parameters))
	}

	if fn.Parameters[0].String() != "x" {
		t.Fatalf("want 'x' paramter, got '%s'", fn.Parameters[0].String())
	}

	expectedBody := "(x + 2)"

	if fn.Body.String() != expectedBody {
		t.Fatalf("want fn body '%s', got '%s'", expectedBody, fn.Body.String())
	}
}
...
func TestStringConcatenation(t *testing.T) {
	...
	if str.Value != "Hello World!" {
		t.Errorf("want %q string literal, got %q", "Hello World!", str.Value)
	}
}
...
func TestArrayLiterals(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("want array of len 3, got %d", len(result.Elements))
	}
...
`
	...
	if !ok {
		t.Fatalf("want hash, got %T", evaluated)
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("unexpected number of pairs, want 6, got %d", len(result.Pairs))
	}
...
func testIntegerObject(t *testing.T, obj object.Object, expected int64) bool {
	...
	if !ok {
		t.Errorf("want integer, got %T", obj)
		return false
	}

	if result.Value != expected {
		t.Errorf("want %d, got %d", expected, result.Value)
		return false
	}
...
func testBooleanObject(t *testing.T, obj object.Object, expected bool) bool {
	...
	if !ok {
		t.Errorf("want boolean, got %T", obj)
		return false
	}

	if result.Value != expected {
		t.Errorf("want %t, got %t", expected, result.Value)
		return false
	}
